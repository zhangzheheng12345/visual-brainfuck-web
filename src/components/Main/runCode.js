...
export function runCode(ctx, callback) {
  ...
  function runSingleCmd() {
    if (code[ctx.cindex] == '+') {
      ...
      if (ctx.data[ctx.ptr]) ctx.stack.push(ctx.cindex)
      else {
        ctx.cindex++
        let count = 1
      ...
      if (ctx.data[ctx.ptr]) ctx.cindex = ctx.stack[ctx.stack.length - 1]
      else ctx.stack.pop()
    } else if (code[ctx.cindex] == ',') {
      getInput()
    } else if (code[ctx.cindex] == '.') {
